...
When enabled, the content-addressed cache stores only schema-validated normalized observations and integrity metadata from audited analyzers. TypeScript and lint entries also store private dependency fingerprints: content digests, scoped path identifiers, directory listings, and missing-file observations. Dependency source bytes remain in memory for analysis and are not saved in this cache. Zedbee rechecks the dependency fingerprints before reusing results; incomplete capture disables caching for that result. Dead-code observations remain uncached because Knip's complete dependency inputs are not yet captured. New or unknown checks remain uncached until their inputs are audited. Installed engine identities come from the package metadata shipped with the active installation; missing identity metadata disables caching for that check. The cache must never store source, detected secret values, raw analyzer output, absolute snapshot paths, or online response bodies. Corrupt, ineligible, or incompatible entries are treated as misses.
