...
When automatic terminal output exceeds `reporting.terminalFindingLimit`, Zedbee writes the complete JSON report to protected operating-system temporary storage before printing its path. Each managed directory is restricted to the current OS user and each report is written with owner-only permissions. `reporting.temporaryReportRetention` defaults to 5 subsequent runs, but the operating system may delete temporary files early; the path is a short-lived handoff, not durable archival storage.
