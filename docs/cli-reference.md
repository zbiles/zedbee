...
Detailed setup, commands, settings, and troubleshooting for Zedbee.
...
- [Analyzer boundaries](#managed-analyzer-boundary)
- [Output and reports](#output)
...
Stage the configuration and the files you intend to commit before scanning.
Zedbee reads staged content, not later working-file edits. With nothing staged,
...
[privacy guide](privacy.md#update-notifications) for the network and cache details.
...
[managed-fixes guide](managed-fixes.md).
...
[complete managed-fix contract](managed-fixes.md).
...
`zedbee checks` uses the same responsive behavior. Checks uses the yellow Zedbee frame with one full-width `CHECKS` panel in a wide interactive terminal, showing each check's severity, applicability, engine, targets, execution details, network use, and limitations. Mixed managed/project formatting policies list every detected project root and Prettier version; JSON exposes the same inventory as `effectiveEngines`. `zedbee checks --format text` forces the complete plain view, while `zedbee checks --format json` keeps deterministic, ANSI-free machine output. Narrow terminals, redirected output, CI, and `TERM=dumb` use plain text; `--no-color` keeps an eligible framed layout without semantic colors.
...
`zedbee doctor` inspects configuration and environment without running a scan and, by default, inspects any project Prettier setup as data only. Pass `--trust-project-prettier` to have its formatting probe run the project's installed Prettier through the same installation and snapshot resolution that scans use. `scan` and `fix` accept the same flag: it grants invocation-only consent so a fresh, trusted CI checkout can run project formatting without interactive init; a clean clone without the flag reports the formatting check incomplete instead of executing anything.
...
An entry can instead use `"syntax": "gitignore"` with a repository-relative `basePath` (`"."` for the root). Its `files` array then contains ordered ignore-file lines, including comments, directory rules, escapes, and `!` re-inclusions. Each entry is evaluated independently; a re-inclusion cannot undo another entry's exclusion. This syntax uses Prettier's case-insensitive gitignore matching. `init --formatting copy` uses it to preserve project-root `.prettierignore` and `.gitignore` files for formatting only. Repeat copy updates generated entries and preserves handwritten exclusions; switching away from copy removes its generated exclusions. These are saved rules, not live references to the original files.

`overrides[].excludeFiles` has a different purpose: it prevents that override from applying while leaving normal checks enabled. Its patterns use the same repository-relative glob syntax as `overrides[].files`.
...
Explicit `--format text`, `--format json`, and `--format sarif` output remains complete, has no finding cap, and writes no sidecar. Redirect an explicit format when you need a durable saved location. SARIF is the complete, deterministically ordered enterprise export and retains Zedbee's normal exit status. See the [reporting guide](reporting.md) for the full contract, including incomplete-scan notifications, retention, and source-excerpt behavior.
...
Scan time depends on project size, enabled checks, baseline comparison, and validated cache reuse. A running service does not mean every analyzer remains loaded: some engines retire after a session or snapshot side. These lifecycle choices do not guarantee faster scans. See [safe analyzer diagnostics](reporting.md#safe-analyzer-diagnostics) to investigate a slow or incomplete job.
...
See [the complete check matrix](checks.md), [support matrix](support.md), [privacy and data handling](privacy.md), and [security policy](../SECURITY.md).
