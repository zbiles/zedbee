...
Zedbee does not load a project's native analyzer config unless a project formatting engine is explicitly trusted (see [formatting engines](checks.md#formatting-engines)). Prettier, ESLint, React, Hooks, and JSX accessibility behavior comes from Zedbee's managed settings and bundled rules; custom plugins and executable project configuration are outside the supported boundary, except that trusted project mode runs the project's own Prettier, configuration, and plugins at the supported version boundary. Rule options follow the analyzer and plugin versions pinned by the installed Zedbee release and can change when Zedbee upgrades its managed engines. `zedbee checks` displays effective formatting settings and reports the detected project Prettier version whenever the root policy or a path override selects the project engine.

Trusted project formatting supports declared Prettier 3 installations resolved through ordinary npm or Yarn `node_modules` layouts and pnpm installations whose repository link points into the pnpm store. Yarn Plug'n'Play, custom Prettier wrapper commands, and CLI-only options such as a custom `--ignore-path` are not reproduced. Configuration files and relative helpers must be contained in the selected Git snapshot; a configuration resolved outside the disposable mirror is rejected before it is imported. Project mode uses the project's root `.prettierignore` and `.gitignore`. Copy mode imports project-root ignore files once as formatting-only exclusions; custom ignore paths are not imported, and later changes to the original ignore files are not synchronized.

Relative snapshot symlinks to contained regular files, including configuration files and link chains, are preserved in the disposable mirror. Absolute, escaping, broken, cyclic, and directory snapshot links make project formatting incomplete rather than silently dropping their configuration or reading live files.

The project formatter receives a reduced environment and a temporary `HOME`, but it is not an operating-system sandbox. Configuration and plugins still run with the invoking account's filesystem and network permissions, and their behavior can vary with Node.js, the operating system, environment variables, or external services. Only enable project formatting after reviewing the code selected by the scan. CI should grant `--trust-project-prettier` only in a trusted workflow and must not add it automatically for unreviewed pull-request code.
