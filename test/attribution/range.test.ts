...
describe("attributeByRange", () => {
  it("attributes a finding whose reported range overlaps a staged line", () => {
    expect(
      attributeByRange(finding("src/value.ts", 4, 6), changeSet),
    ).toMatchObject({
      id: "finding-1",
      attribution: {
        kind: "range-overlap",
  ...
  it("does not attribute a finding elsewhere in a changed file", () => {
    expect(
      attributeByRange(finding("src/value.ts", 8, 9), changeSet).attribution,
    ).toEqual({
      kind: "none",
      staged: false,
      evidence: [],
  ...
  it("does not attribute a finding without a precise line", () => {
    expect(
      attributeByRange(finding("src/value.ts"), changeSet).attribution.staged,
    ).toBe(false);
    expect(attributeByRange(finding(), changeSet).attribution.staged).toBe(
      false,
    );
  });
  ...
  it("normalizes Windows separators before matching", () => {
    expect(
      attributeByRange(finding("src\\value.ts", 5, 5), changeSet).attribution
        .staged,
    ).toBe(true);
  });
});
