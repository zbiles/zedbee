...
A useful report identifies the affected Zedbee version or commit, platform, security boundary, expected behavior, observed behavior, and a synthetic proof of concept. Reports about a bundled analyzer should also name its engine and pinned version.

## Security boundaries

Zedbee treats repository content and analyzer output as untrusted. It scans isolated Git-index snapshots, uses inert managed configuration, rejects executable project analyzer configuration, runs managed subprocesses without a shell, verifies managed executable/configuration checksums, and removes temporary reports and snapshots after every outcome.
...
- executing a managed binary or rules file that does not match its reviewed checksum;
