...
Zedbee may cache content-addressed, normalized observations for audited local analyzers. Cache keys include source mode, the exact baseline and target identity, snapshot inventories, policy, workspace/config inputs, installed engine package identities, and runtime platform. TypeScript and lint also capture the dependency files and resolution state they use, and recheck those inputs before reusing saved results. If complete dependency capture is unavailable, those checks run without saving or reusing results. Dead-code observations remain uncached because Knip's complete dependency inputs are not yet captured; unknown checks also default to uncached. Missing engine package metadata, cache failures, and corruption are misses and never reduce coverage. Source, raw analyzer output, Secretlint observations, OSV results, secrets, and online response bodies are never stored in the observation cache. Cached and uncached reports are required to remain semantically identical.
