...
export interface ResolvedPolicyOverride {
  ...
  /** Authorship marker carried for repeat setup; never part of policy merging. */
  readonly generated?: "prettier-copy" | "prettier-engine";
}

export interface PathExclusion {
  readonly syntax?: "gitignore";
  readonly basePath?: string;
  readonly generated?: "prettier-copy";
  readonly files: readonly string[];
...
const formattingPolicyObjectSchema = z
  .object({
    ...commonPolicyFields,
    engine: z.enum(FORMATTING_ENGINES).optional().meta({
      description:
    ...
    /** Authorship marker; resolved policy drops it. It exists so `zedbee init`
     * can replace only what it wrote without deleting hand-authored policy. */
    generated: z.literal("prettier-copy").optional(),
  })
  .strict()
  .superRefine((policy, context) => {
...
const globPathExclusionSchema = z
  .object({
...
const gitignorePathExclusionSchema = z
  .object({
    syntax: z.literal("gitignore"),
    basePath: z
      .string()
      .min(1)
      .max(4096)
      .refine(
        ...
        "Expected a repository-relative directory or .",
      ),
    files: z
      .array(
        z
          .string()
          .max(262144)
          .regex(/^[^\r\n\u0000]*$/u, "Expected one ignore rule per entry"),
      )
      .max(10000)
      .refine(
        (rules) =>
          rules.reduce((size, rule) => size + rule.length + 1, 0) <= 262144,
        "Ignore rules exceed 256 KiB",
      ),
    checks: pathExclusionCheckIdSchema,
    reason: pathExclusionReasonSchema,
    generated: z.literal("prettier-copy").optional(),
  ...
  .strict();
const pathExclusionSchema = z.union([
  globPathExclusionSchema,
  gitignorePathExclusionSchema,
]);
...
const policyOverrideSchema = z
  .object({
    ...
    excludeFiles: z.array(repositoryRelativeGlobSchema).optional().meta({
      description:
        "Files that must not receive this override; normal checks still apply.",
    }),
    ...
    /** Authorship marker: `zedbee init` replaces its own generated entries on
     * repeat setup and never touches unmarked, hand-authored overrides. */
    generated: z.enum(["prettier-copy", "prettier-engine"]).optional().meta({
      description:
        "Marks this override as generated by Zedbee setup; repeat setup replaces marked entries instead of duplicating them.",
    }),
  })
  .strict();
